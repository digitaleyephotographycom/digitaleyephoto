...
// Reliably resolve the configured admin bcrypt password hash or configured password.
// Next.js dotenv-expand may evaluate $ in .env as variables, so if process.env
// was truncated, read the raw unexpanded value from .env.local as a fallback.
export function getAdminPasswordHash() {
    ...
    if (fs.existsSync(envPath)) {
      ...
      for (const line of content.split("\n")) {
        ...
        if (trimmed.startsWith("ADMIN_PASSWORD=")) {
          const raw = trimmed.slice("ADMIN_PASSWORD=".length).trim().replace(/^['"]|['"]$/g, "");
          if (raw) return raw;
        }
      }
    }
  ...
  return (process.env.ADMIN_PASSWORD_HASH || process.env.ADMIN_PASSWORD || "").trim();
}
